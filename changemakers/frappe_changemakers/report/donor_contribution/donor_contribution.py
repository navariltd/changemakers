...
class DonorContributionReport:
    ...
    def get_data(self):
        ...
        if filters.get("donor"):
            ...
            donation_filters["donor"] = filters.get("donor")
        ...
        if isinstance(donation_filters.get("date"), list) and len(donation_filters["date"]) > 2:
            dates = donation_filters.pop("date")[1::2]
            donation_filters["date"] = ["between", dates]
        ...
        items = frappe.get_all(
            ...
            fields=["parent", "amount", "percentage", "recipient_type", "recipient", "project", "cost_center"],
            order_by="parent, idx"
        )
        ...
        for donor, donor_donations in donations_by_donor.items():
            ...
            for donation in donor_donations:
                ...
                row = {
                    ...
                    "recipient_type": first_item.recipient_type if first_item else None,
                    "recipient": first_item.recipient if first_item else None,
    ...
    def get_chart_data(self):
        ...
        for row in self.data:
            ...
            elif current_donor_name and row.get("amount") is not None: 
    ...
    def get_report_summary(self):
        ...
        calculated_total_distributed = sum((row.get("amount") or 0) for row in self.data if row.get("amount") is not None and row.get("amount") != "")

        self.report_summary = [
            {"label": "Total Pledged Amount", "value": total_pledged, "datatype": "Currency", "indicator": "blue"},
            {"label": "Total Paid Amount", "value": total_paid, "datatype": "Currency", "indicator": "green"},
            {"label": "Total Distributed Amount", "value": calculated_total_distributed, "datatype": "Currency", "indicator": "orange"}
        ]